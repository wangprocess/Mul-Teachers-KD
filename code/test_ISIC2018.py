...
from utils import transforms, meterics
from networks.net_factory import net_factory
from dataloaders.datasets import DataSet_ISIC_HQ_LQ_Test
...
parser.add_argument('--model_path', type=str,
                    default='../model/Dermopathy/MTKD/UNext/XXXXXXXXXXX.pth', help='the test model path')
...
def test(args, snapshot_path):
    ...
    db_test = DataSet_ISIC_HQ_LQ_Test(base_dir=args.root_path, split=args.data_quality, num=args.data_num, transform=transforms.Train_Transforms)
    ...
    with torch.no_grad():
        ...
        for (img_test, label_test) in tqdm(test_loader):
            ...
            test_acc += meterics.mean_accuracy(logits, label_test).cpu()

        AIT = total_time / num_samples
        test_auc = roc_auc_score(LABELS, PROBS, average='macro', multi_class='ovr')
        one_hot = torch.eye(7)
        AUROCs, Accus, Senss, Specs, pre, F1 = meterics.compute_metrics_test(one_hot[LABELS], PROBS, competition=True)
