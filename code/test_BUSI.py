...
from utils import transforms, meterics2
from networks.net_factory import net_factory
from dataloaders.datasets import DataSet_BUSI_HQ_LQ_Test

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default='../data/Breast', help='the ROOT path/the path of dataset')
parser.add_argument('--exp', type=str,
                    default='Breast/MTKD', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='ShiftMLP_s', help='model_name')
parser.add_argument('--model_path', type=str,
                    default='../model/Breast/MTKD/XXXXXXXXXXX.pth', help='the test model path')
...
parser.add_argument('--data_quality', type=str,
                    default='HQ-Test', help='data quality name')
...
def test(args, snapshot_path):
    ...
    db_test = DataSet_BUSI_HQ_LQ_Test(base_dir=args.root_path, split=args.data_quality, num=args.data_num, transform=transforms.BUSI_Val_Transforms)
    ...
    with torch.no_grad():
        ...
        for (img_test, label_test) in tqdm(test_loader):
            ...
            test_acc += meterics2.mean_accuracy(logits, label_test).cpu()

        AIT = total_time / num_samples
        test_auc = roc_auc_score(LABELS, PROBS, average='macro', multi_class='ovr')
        one_hot = torch.eye(3)

        AUROCs, Accus, Senss, Specs, pre, F1 = meterics2.compute_metrics_test(one_hot[LABELS], PROBS, competition=True)
